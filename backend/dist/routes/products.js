...
router.get('/', auth_1.optionalAuth, async (req, res) => {
    try {
        const { page = 1, limit = 20, category, search, seller_id, min_price, max_price, sort = 'created_at', order = 'desc' } = req.query;
...
router.get('/:id', auth_1.optionalAuth, async (req, res) => {
    try {
      ...
      `)
        ...
        logger_1.logger.error('Get product error:', error);
        res.status(500).json({
            success: false,
...
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['seller', 'admin']), async (req, res) => {
    try {
        const { title, description, price, currency = 'USD', stock_quantity = 0, category_id, images = [] } = req.body;
        ...
        if (images.length > 0) {
            ...
            const { error: imageError } = await supabase_1.supabase
                .from('product_images')
                .insert(imageData);
            if (imageError) {
                logger_1.logger.error('Error adding product images:', imageError);
            }
        }
        res.status(201).json({
            success: true,
...
router.put('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, price, currency, stock_quantity, category_id, is_active } = req.body;
        // Check if user owns the product or is admin
        const { data: existingProduct, error: fetchError } = await supabase_1.supabase
            .from('products')
            .select('seller_id')
            .eq('id', id)
            .single();
...
router.get('/my/products', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const offset = (Number(page) - 1) * Number(limit);
        const { data: products, error, count } = await supabase_1.supabase
            .from('products')
            .select(`
        *,
        categories(name, slug),
      ...
      `, { count: 'exact' })
        ...
        logger_1.logger.error('Get user products error:', error);
        res.status(500).json({
            success: false,
