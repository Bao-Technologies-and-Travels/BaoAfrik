                ...
                body: """
                    <html>
                        <body style="font-family: Arial, sans-serif; line-height: 1.5; color: #333;">
                            ...
                            <p><strong>Job:</strong> ${env.JOB_NAME}</p>
                            <p><strong>Status:</strong> <span style="color: ${currentBuild.currentResult == 'SUCCESS' ? 'green' : 'red'};">${currentBuild.currentResult}</span></p>
                            <p><strong>Changes made:</strong>Encrypt messages when sending and storing from database, decrypt when fetching. Deployed front-end changes to product details page, mobile version of messages feature. Fetch seller details from API. Displayed real products from API in homepage</p>
